...
RUN apt-get update && apt-get install -y --no-install-recommends python3 python3-pip ca-certificates && pip3 install --no-cache-dir --break-system-packages yt-dlp && apt-get clean && rm -rf /var/lib/apt/lists/*
WORKDIR /app
...
# Copy the whole project first so postinstall (scripts/setup.mjs) exists when
# npm ci runs. Copying only package*.json broke the install because the
# postinstall script file did not exist yet at that step.
COPY . .
RUN npm ci
RUN npm run build
